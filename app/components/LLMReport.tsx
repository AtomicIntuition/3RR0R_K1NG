...
export function LLMReport({ report }: LLMReportProps) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden shadow-sm">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 border-b border-gray-800 bg-gray-800">
      ...
      <div className="p-4">
        <div className={`relative ${!expanded && needsExpand ? 'max-h-48' : ''} overflow-hidden`}>
          <pre
            ref={contentRef}
            className="text-sm text-gray-300 whitespace-pre-wrap font-mono bg-gray-950 p-4 rounded-xl leading-relaxed overflow-x-auto border border-gray-800"
          >
            {report}
          </pre>
        ...
        {needsExpand && (
          <button
            ...
            className="mt-4 w-full py-3 px-4 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-xl text-sm font-medium text-emerald-500 hover:text-emerald-400 transition-all flex items-center justify-center gap-2 active:scale-[0.98]"
          >
            {expanded ? (
              <>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="px-4 pb-4">
        <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3">
          <p className="text-xs text-gray-400">
            <strong className="text-emerald-500">How to use:</strong> Click &quot;Copy for AI&quot;, then paste into your favorite AI assistant
