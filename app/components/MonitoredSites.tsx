...
export function MonitoredSites() {
  ...
  return (
    <div className="bg-void-50 rounded-lg border border-void-100 overflow-hidden">
      <div className="px-4 sm:px-6 py-4 border-b border-void-100 flex items-center justify-between">
        <div>
        ...
        {isPro && activeSites.length < maxSites && (
          <button
            ...
            className="px-4 py-2 bg-terminal text-void font-bold rounded hover:bg-terminal-bright transition-colors text-sm"
          >
            + Add Site
          </button>
        )}
      ...
      {!isPro && (
        <div className="p-6 text-center">
          ...
          <Link
            href="/pricing"
            className="inline-block px-6 py-2 bg-terminal text-void font-bold rounded hover:bg-terminal-bright transition-colors"
          >
            Upgrade to Pro
          </Link>
        </div>
      ...
      {/* Add site form */}
      {isPro && showAddForm && (
        <div className="p-4 sm:p-6 border-b border-void-100 bg-void/50">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              className="px-6 py-2 bg-terminal text-void font-bold rounded hover:bg-terminal-bright transition-colors disabled:opacity-50 whitespace-nowrap"
            >
      ...
      {isPro && (
        <>
            ...
            <div className="divide-y divide-void-100">
              {activeSites.map((site) => (
                <div key={site.id} className="px-4 sm:px-6 py-4 flex items-center gap-4">
                  {/* Score badge */}
                  <div className="flex-shrink-0 w-12 h-12 rounded-lg bg-void flex items-center justify-center">
                    {site.last_grade ? (
                  ...
                  {site.last_score !== null && (
                    <div className="text-right hidden sm:block">
                      <p className="text-2xl font-bold text-gray-200">{site.last_score}</p>
                  ...
                  <button
                    ...
                    className="px-3 py-1 text-sm text-gray-400 hover:text-danger hover:bg-danger/10 rounded transition-colors"
                  >
                    Remove
                  </button>
                </div>
          ...
          {activeSites.length > 0 && activeSites.length < maxSites && !showAddForm && (
            <div className="px-4 sm:px-6 py-3 border-t border-void-100 text-center">
              <button
                onClick={() => setShowAddForm(true)}
                className="text-sm text-terminal hover:underline"
              >
