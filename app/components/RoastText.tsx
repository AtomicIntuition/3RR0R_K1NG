...
const PERSONA_CONFIG: Record<string, { name: string; emoji: string }> = {
  hacker: { name: '3RROR_K1NG', emoji: '💀' },
  gordon: { name: 'Gordon Websy', emoji: '👨‍🍳' },
  parent: { name: 'Disappointed Parent', emoji: '😔' },
  interviewer: { name: 'Tech Interviewer', emoji: '🤔' },
  drill: { name: 'Drill Sergeant', emoji: '🎖️' },
  meme: { name: 'Meme Lord', emoji: '🗿' },
  therapist: { name: 'Web Therapist', emoji: '🛋️' },
};

export function RoastText({ title, body, score, persona = 'hacker', className }: RoastTextProps) {
  const [isRevealed, setIsRevealed] = useState(false);
  const personaInfo = PERSONA_CONFIG[persona] || PERSONA_CONFIG.hacker;
